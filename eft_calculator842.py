...
class EFT_calculator:
    def __init__(self, order=2):
        self.mol = Water()
        self.grid = Grid(centre=(0.,0.,0.), size=12.0, symmetry=2)
    ...
    def gen_atomic_coors(self):
        for conf in self.grid._iter_conf():
            ...
            coors = self._hopf2Atomic(conf.position, conf.vector, conf.angle)
            yield conf.idx, coors

    # Construct atomic coordinates for a pair from grid coordinate
    def _hopf2Atomic(self, Xcom, vector, angle):
        q = tools.hopf2q(vector, angle)
        log.write(' V:'+'%5.2f '*3%tuple(vector)+'A:'+ '%5.2f '%(angle)+'q:'+ '%5.2f '*4%tuple(q)+'\n')
